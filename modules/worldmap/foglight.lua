--[[
		The hard work for this is down to Semlar and his
		Foglight addon. Modified to integrate into my UI
--]]
...
local moads = { "Disabled", "Hybrid", "All Terrain", "No Terrain" }
...
menu:SetFrameLevel(12)
menu:SetAlpha(0.8)

--[[

--]]
M.FogLightSetMode = function(mode)
	if not moads[mode + 1] then
		mode = 1
	end

	UIDropDownMenu_SetSelectedID(foglightmenu, mode + 1)

	if (mode == 0) then -- disabled
		overlayFrame:Hide()
	elseif mode == 1 then -- hybrid
		overlayFrame:Show()
		for _,tx in pairs(terrainTextures) do
			tx:SetDrawLayer("ARTWORK", -2)
		end
		terrainFrame:SetShown(showTerrain)
	elseif mode == 2 then -- all terrain (3)
		overlayFrame:Show()
		for _,tx in pairs(terrainTextures) do
			tx:SetDrawLayer("ARTWORK", 2)
		end
		terrainFrame:SetShown(showTerrain)
	elseif mode == 3 then -- no terrain (4)
		terrainFrame:Hide()
		overlayFrame:Show()
	end

	T.db["worldMap"].fogLightMode = mode
end

local function menuOnClick(self)
	M.FogLightSetMode(self:GetID() - 1)
end
...
local function SuperSetTexture(texture, texturePath, textureWidth, textureHeight)
	...
	if cached then
		...
		if #TextureQueue == 1 then
			TextureUpdater:SetScript("OnUpdate", UpdateTextures)
		end
		texture:SetTexture(texturePath)
	end
end
...
local function update_overlays()
						...
						for j=1,numTexturesTall do
							...
							for k=1,numTexturesWide do
								...
								if not overlayTextures[texNum] then
									...
									texture:SetVertexColor(T.db["worldMap"].unexploredColor.r or 0.7, T.db["worldMap"].unexploredColor.g or 0.7, T.db["worldMap"].unexploredColor.b or 0.7, T.db["worldMap"].unexploredColor.a or 0.9)
	...
	if info and not isMicroDungeon then
		...
		if TLx and TLx ~= 0 and TERRAIN_MAPS[terrainMapID] and areaID ~= 640 and floorNum == 0 then
			...
			for y=1,numTilesY do
				for x=1,numTilesX do
					...
					if not terrainTextures[n] then
						terrainTextures[n] = terrainFrame:CreateTexture(nil, "ARTWORK", nil, T.db["worldMap"].fogLightMode == 2 and 2 or -2)
						--terrainTextures[n]:SetNonBlocking(true)
						terrainTextures[n]:SetAlpha(T.db["worldMap"].terrainAlpha)
					end
			...
			showTerrain = true
			if T.db["worldMap"].fogLightMode == 1 or T.db["worldMap"].fogLightMode == 2 then
				terrainFrame:Show()
			end
		...
		CoordsHolder.playerCoords:SetPoint("BOTTOMLEFT", WorldMapScrollFrame, "BOTTOMLEFT", 120, 8)
	else
		menu:Hide()
		--if activeMode == button3 then WorldMapHighlight:SetParent(WorldMapDetailFrame) end
		terrainFrame:Hide()
		...
		CoordsHolder.playerCoords:SetPoint("BOTTOMLEFT", WorldMapScrollFrame, "BOTTOMLEFT", 8, 8)
	end
end
