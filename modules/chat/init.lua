...
local Chat = T:NewModule("Chat", "AceEvent-3.0", "AceTimer-3.0", "AceHook-3.0")
...
do
	Chat.GetBNFriendColor = function(name, id)
	...
	Chat.ConcatenateTimeStamp = function(msg)
	...
	Chat.ShortChannel = function(self)
	...
	Chat.ChatFrame_MessageEventHandler = function(self, event, ...)
		if (strsub(event, 1, 8) == "CHAT_MSG") then
			...
			elseif (type == "CHANNEL_NOTICE") then
				...
				globalString = Chat.ConcatenateTimeStamp(globalstring)
			...
			elseif ( type == "BN_INLINE_TOAST_CONVERSATION" ) then
				...
				body = body:gsub("|Hchannel:(.-)|h%[(.-)%]|h", Chat.ShortChannel)
